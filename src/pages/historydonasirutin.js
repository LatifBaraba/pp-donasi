...
import { fetchAllHistoryRutinDonation, fetchHistoryRutinDonation } from "./DonasiRutin/Controller/detaildonasi/action";
...
const ListProgram = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchHistoryRutinDonation(token, id));
  }, []);
        ...
        <div className="col-md-6">
          <div className="article-detail">
            <div className="article-heading">
              <h2 className="article-title">{"Donasi "}({rutinhistorydata.length})</h2>
            </div>
          </div>
        </div>
        </center>
        {rutinhistorydata.map((data, idx) => (
